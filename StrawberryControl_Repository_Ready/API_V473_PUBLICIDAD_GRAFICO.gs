...
/**
 * CAPA DE RENDIMIENTO V4.6
 * - Cache corto para dashboards/listados.
 * - Cache de validacion de sesion para evitar leer SESIONES/USUARIOS en cada click.
 * - Escritura de ULTIMA_ACTIVIDAD como maximo cada 5 minutos.
 * - El procesamiento de vencimientos pasa a un trigger de 15 minutos.
 */
const SC_PERF46 = {
  DASHBOARD_TTL: 30,
  ORDERS_TTL: 30,
  ...
  CACHE_EPOCH_PROP: 'SC_V46_CACHE_EPOCH',
  SESSION_EPOCH_PROP: 'SC_V46_SESSION_EPOCH'
};

function apiCache46_(){ return CacheService.getScriptCache(); }
function apiEpoch46_(){ return PropertiesService.getScriptProperties().getProperty(SC_PERF46.CACHE_EPOCH_PROP) || '1'; }
function apiSessionEpoch46_(){ return PropertiesService.getScriptProperties().getProperty(SC_PERF46.SESSION_EPOCH_PROP) || '1'; }
function apiInvalidarCachesV46_(){ PropertiesService.getScriptProperties().setProperty(SC_PERF46.CACHE_EPOCH_PROP, String(Date.now())); }
function apiInvalidarCacheSesionesV46_(){ PropertiesService.getScriptProperties().setProperty(SC_PERF46.SESSION_EPOCH_PROP, String(Date.now())); }
function apiKey46_(prefix, params){
  const p = params || {};
  const clean = {};
  Object.keys(p).sort().forEach(function(k){ if(k !== 'token' && k !== 'fresh') clean[k] = p[k]; });
  return 'SC46:' + apiEpoch46_() + ':' + prefix + ':' + apiHashSimple_(JSON.stringify(clean)).substring(0,24);
}
function apiCacheGetJson46_(key){
  try { const raw=apiCache46_().get(key); return raw ? JSON.parse(raw) : null; } catch(e){ return null; }
}
function apiCachePutJson46_(key, value, ttl){
  try { const raw=JSON.stringify(value); if(raw.length < 95000) apiCache46_().put(key, raw, Math.max(1, ttl)); } catch(e){}
  return value;
}
function apiFresh46_(params){ return apiBooleano_(params && params.fresh); }
function apiSessionKey46_(hash){ return 'SC46S:' + apiSessionEpoch46_() + ':' + String(hash||'').substring(0,48); }
function apiSessionCachePut46_(hash,user,expiresAt){
  try {
    const ttl=Math.max(1,Math.min(SC_PERF46.SESSION_TTL,Math.floor((expiresAt.getTime()-Date.now())/1000)));
    apiCache46_().put(apiSessionKey46_(hash),JSON.stringify({user:user,expiresAt:expiresAt.toISOString()}),ttl);
  } catch(e){}
}
function apiSessionCacheGet46_(hash){
  try {
    const raw=apiCache46_().get(apiSessionKey46_(hash)); if(!raw)return null;
    const v=JSON.parse(raw), exp=new Date(v.expiresAt);
    if(!v.user || isNaN(exp.getTime()) || exp.getTime()<=Date.now())return null;
    return {user:v.user,expiresAt:exp};
  } catch(e){ return null; }
}
function apiSessionCacheRemove46_(hash){ try{ apiCache46_().remove(apiSessionKey46_(hash)); }catch(e){} }

/** Ejecutar UNA VEZ despues de pegar API V4.6. Mueve vencimientos fuera de las pantallas. */
function instalarRendimientoV46(){
  ScriptApp.getProjectTriggers().forEach(function(t){
    if(t.getHandlerFunction()==='procesarVencimientosV46') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('procesarVencimientosV46').timeBased().everyMinutes(15).create();
  apiInvalidarCachesV46_();
  console.log('RENDIMIENTO V4.6 ACTIVADO: cache + vencimientos cada 15 minutos.');
  return true;
}
function procesarVencimientosV46(){
  try { v45ProcesarVencimientos_(); } finally { apiInvalidarCachesV46_(); }
}

/** Ejecutar una vez junto con actualizarNegocioV47(). */
function instalarRendimientoV47(){
  instalarRendimientoV46();
  if (typeof asegurarActivadorSincronizacionV47_ === 'function') {
    asegurarActivadorSincronizacionV47_();
  }
  apiInvalidarCachesV46_();
  console.log('V4.7.1 ACTIVADA: rendimiento + vencimientos + sincronizacion incremental Hoja 1 -> GESTION PEDIDOS.');
  return true;
}
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'getOrder': return apiJson_(apiGetOrderPayloadV471_(params.pedido));
      ...
      case 'listAdvertising':
        apiExigirAdmin_(user);
        return apiJson_({ ok: true, items: apiListarGastos_(params), tipoCambioPublicidad: apiTipoCambioPublicidadV472_() });
...
function crearOResetearAdministrador() {
  ...
  console.log('ADMINISTRADOR V4.7 CREADO / RESETEADO');
...
function apiLogin_(params) {
  ...
  apiSessionCachePut46_(tokenHash, user, expira);
...
function apiValidarSesion_(token) {
  ...
  // Via rapida: durante unos segundos no tocamos Google Sheets.
  const cached = apiSessionCacheGet46_(hash);
  ...
  for (let i=0;i<data.length;i++) {
    ...
    // Antes se escribia ULTIMA_ACTIVIDAD en cada click. Ahora maximo cada 5 minutos.
    const ultimo=aFecha_(f[6]);
    if(!ultimo || ahora.getTime()-ultimo.getTime() >= SC_PERF46.SESSION_TOUCH_MINUTES*60000){
      sh.getRange(i+2,7).setValue(ahora);
    }
    const user={ id:usr.datos[0], usuario:usr.datos[1], nombre:usr.datos[2], rol:usr.datos[3], activo:true };
    apiSessionCachePut46_(hash,user,expira);
    return user;
  }
...
function apiInvalidarSesionesUsuario_(id){if(!id)return;apiInvalidarCacheSesionesV46_();const sh=v45Aux_(SC_WEB45.HOJA_SESIONES);if(sh.getLastRow()<2)return;const d=sh.getRange(2,1,sh.getLastRow()-1,8).getValues();for(let i=0;i<d.length;i++)if(String(d[i][1])===String(id)&&apiBooleano_(d[i][7]))sh.getRange(i+2,8).setValue(false);}
...
function v45MetaExpirarAtencion_(m,cfgOpcional){const cfg=cfgOpcional||leerConfigWebV45_(),who=String(m[5]||''),d=aFecha_(m[6]);if(who&&d&&Date.now()-d.getTime()>=cfg.minutosBloqueo*60000){m[5]='';m[6]='';return true;}return false;}
...
function apiGetOrderPayloadV471_(pedido){
  const cfg=leerConfigWebV45_();
  const r=apiObtenerRegistroPedido_(pedido);
  const m=v45MetaRegistro_(pedido,false).datos;
  if(v45MetaExpirarAtencion_(m,cfg)){
    const mr=v45MetaRegistro_(pedido,true);mr.datos=m;v45GuardarMeta_(mr);
  }
  return {ok:true,order:apiFilaAOrdenV45_(r.datos,m,cfg),history:apiListarHistorialLlamadas_(pedido,100),maxAttempts:cfg.maxIntentos};
}

function apiListarPedidosV45_(params) {
  const cacheKey=apiKey46_('orders',params);
  if(!apiFresh46_(params)){
    const cached=apiCacheGetJson46_(cacheKey);
    if(cached)return cached;
  }

  const hoja=apiObtenerHoja_();
  const lectura=leerGestion_(hoja);
  const metaMap=v45MetaMap_();
  // Se lee una sola vez. Antes se consultaba CONFIGURACION WEB por cada pedido.
  const cfgWeb=leerConfigWebV45_();

  let orders=lectura.filas.map(function(r){
    const p=String(r.datos[SC_COL.PEDIDO]||'');
    const meta=metaMap.has(p)?metaMap.get(p).datos:v45MetaDefault_(p);
    return apiFilaAOrdenV45_(r.datos,meta,cfgWeb);
  });

  const q=normalizarTexto_(params.q||params.search||''),
        estado=normalizarTexto_(params.estado||''),
        zona=normalizarTexto_(params.zona||''),
        plataforma=normalizarFiltroPlataforma_(params.plataforma||'TODAS');
  ...
  orders=orders.filter(function(o){
    ...
    const f=aFecha_(o.fecha);
    if(desde&&(!f||f<inicioDia_(desde)))return false;
    if(hasta&&(!f||f>finDia_(hasta)))return false;
    return true;
  });

  orders.sort(function(a,b){
    return String(b.fecha||'').localeCompare(String(a.fecha||''))||String(b.pedido).localeCompare(String(a.pedido));
  });

  const total=orders.length,
        page=Math.max(1,Math.floor(numeroSeguro_(params.page)||1)),
        pageSize=Math.min(200,Math.max(10,Math.floor(numeroSeguro_(params.pageSize)||50))),
        start=(page-1)*pageSize;
  const result={items:orders.slice(start,start+pageSize),total:total,page:page,pageSize:pageSize,pages:Math.max(1,Math.ceil(total/pageSize))};
  return apiCachePutJson46_(cacheKey,result,SC_PERF46.ORDERS_TTL);
}
function apiObtenerPedidoV45_(pedido){const r=apiObtenerRegistroPedido_(pedido),m=v45MetaRegistro_(pedido,false).datos,cfg=leerConfigWebV45_();if(v45MetaExpirarAtencion_(m,cfg)){const mr=v45MetaRegistro_(pedido,true);mr.datos=m;v45GuardarMeta_(mr);}return apiFilaAOrdenV45_(r.datos,m,cfg);}
...
function apiDashboardOperativo_(p){
  const params=p||{}, key=apiKey46_('dashboard-operativo',{});
  if(!apiFresh46_(params)){const cached=apiCacheGetJson46_(key);if(cached)return cached;}
  const hoja=apiObtenerHoja_(),meta=v45MetaMap_(),estados={},zonas={LIMA:0,PROVINCIA:0};
  let vencidas=0,llamadasHoy=0,esperando7=0,atendiendo=0;
  const ahora=Date.now(),hoy=inicioDia_(new Date()).getTime(),cfg=leerConfigWebV45_();
  leerGestion_(hoja).filas.forEach(function(r){
    const pedido=String(r.datos[SC_COL.PEDIDO]||''),m=meta.has(pedido)?meta.get(pedido).datos:v45MetaDefault_(pedido);
    const estado=String(r.datos[SC_COL.ESTADO]||'Por llamar'),zona=normalizarZona_(r.datos[SC_COL.ZONA]);
    estados[estado]=(estados[estado]||0)+1; zonas[zona]=(zonas[zona]||0)+1;
    const prox=aFecha_(m[4]); if(prox&&prox.getTime()<ahora&&normalizarTexto_(estado)==='volver a llamar')vencidas++;
    const ul=aFecha_(m[3]); if(ul&&inicioDia_(ul).getTime()===hoy)llamadasHoy++;
    const who=String(m[5]||''),since=aFecha_(m[6]); if(who&&since&&ahora-since.getTime()<cfg.minutosBloqueo*60000)atendiendo++;
    const ea=aFecha_(m[8]); if(ea&&normalizarTexto_(estado)==='esperando adelanto'&&ahora-ea.getTime()>=(cfg.diasEsperaAdelanto-1)*86400000)esperando7++;
  });
  const result={total:Object.values(estados).reduce(function(a,b){return a+b;},0),estados:estados,zonas:zonas,volverALlamarVencidas:vencidas,llamadasHoy:llamadasHoy,esperandoAdelantoProximosAVencer:esperando7,atendiendoAhora:atendiendo,maxIntentos:cfg.maxIntentos,generatedAt:new Date().toISOString()};
  return apiCachePutJson46_(key,result,SC_PERF46.DASHBOARD_TTL);
}
function apiDashboardAdmin_(p){
  const params=p||{}, key=apiKey46_('dashboard-admin',params);
  if(!apiFresh46_(params)){const cached=apiCacheGetJson46_(key);if(cached)return cached;}
  const libro=obtenerLibro_(),resumen=libro.getSheetByName(SC_CONFIG.HOJA_RESUMEN);
  if(resumen&&(params.desde||params.hasta||params.plataforma)){
    if(params.desde)resumen.getRange(SC_RESUMEN.DESDE).setValue(apiFechaParaCelda_(params.desde));
    if(params.hasta)resumen.getRange(SC_RESUMEN.HASTA).setValue(apiFechaParaCelda_(params.hasta));
    if(params.plataforma)resumen.getRange(SC_RESUMEN.PLATAFORMA).setValue(normalizarFiltroPlataforma_(params.plataforma));
    if(params.desde||params.hasta)resumen.getRange(SC_RESUMEN.PERIODO).setValue('PERSONALIZADO');
    actualizarResumenVentasCore_(libro);
  }
  // Ya no cargamos CONFIGURACION en el dashboard porque la vista no la usa.
  const result={operativo:apiDashboardOperativo_(params),financiero:apiLeerResumenV45_()};
  return apiCachePutJson46_(key,result,SC_PERF46.DASHBOARD_TTL);
}
function apiLeerResumenV45_(){
  const sh=obtenerLibro_().getSheetByName(SC_CONFIG.HOJA_RESUMEN);if(!sh)return null;
  // Una sola lectura A1:H27 reemplaza varias llamadas getRange/getValue.
  const all=sh.getRange(1,1,27,8).getValues();
  const real=all.slice(6,13).map(function(r){return r[1];});
  const proy=all.slice(17,24).map(function(r){return r[1];});
  return{periodo:all[2][1]||'',plataforma:normalizarFiltroPlataforma_(all[3][1]),desde:fechaISO_(all[2][4]),hasta:fechaISO_(all[2][7]),real:apiValoresResumenV45_(real),proyeccion:apiValoresResumenV45_(proy),updatedAt:apiFechaHoraISO_(all[26][1])};
}
...
function apiTipoCambioPublicidadV472_(){const cfg=leerConfiguracion_(apiObtenerHoja_());return Math.max(0.01,numeroSeguro_(cfg.tipoCambioPublicidad)||3.5);}
function apiGuardarGasto_(p,user){const fecha=apiFechaParaCelda_(p.fecha);if(!aFecha_(fecha))throw new Error('Fecha obligatoria.');const sh=obtenerLibro_().getSheetByName(SC_CONFIG.HOJA_GASTOS);if(!sh)throw new Error('No existe GASTOS PUBLICIDAD.');let row=Math.floor(numeroSeguro_(p.id));if(row<2)row=Math.max(2,ultimaFilaConDatosEnColumnas_(sh,1,6)+1);const usd=Math.max(0,numeroSeguro_(p.fbDolares)),tc=apiTipoCambioPublicidadV472_(),fb=Math.round((usd*tc)*100)/100,tt=Math.max(0,numeroSeguro_(p.tiktokSoles)),total=Math.round((fb+tt)*100)/100;sh.getRange(row,1,1,6).setValues([[fecha,usd,fb,tt,total,String(p.observacion||'').trim()]]);try{actualizarResumenVentasCore_(obtenerLibro_());}catch(e){}apiRegistrarAuditoria_(user,'GUARDAR_PUBLICIDAD','', 'PUBLICIDAD',fechaISO_(fecha)+' USD '+usd+' x '+tc+' = S/'+fb+'; total S/'+total);apiInvalidarCachesV46_();return{id:String(row),fecha:fechaISO_(fecha),fbDolares:usd,fbSoles:fb,tiktokSoles:tt,totalSoles:total,tipoCambioPublicidad:tc,observacion:String(p.observacion||'').trim()};}
function apiEliminarGasto_(id,user){const sh=obtenerLibro_().getSheetByName(SC_CONFIG.HOJA_GASTOS),row=Math.floor(numeroSeguro_(id));if(!sh||row<2||row>sh.getLastRow())throw new Error('Gasto no encontrado.');sh.getRange(row,1,1,6).clearContent();try{actualizarResumenVentasCore_(obtenerLibro_());}catch(e){}apiRegistrarAuditoria_(user,'ELIMINAR_PUBLICIDAD','', 'PUBLICIDAD',String(id));apiInvalidarCachesV46_();}
...
function apiActualizarConfiguracionV45_(p,user){
  // Reglas operativas de la base web auxiliar.
  ...
  if(aux.getLastRow()>=2){
    const data=aux.getRange(2,1,aux.getLastRow()-1,2).getValues();
    const updates=[];
    data.forEach(function(f,i){
      Object.keys(keys).forEach(function(k){
        if(keys[k]===String(f[0])&&p[k]!==undefined) updates.push({row:i+2,value:Math.max(0,numeroSeguro_(p[k]))});
      });
    });
    updates.forEach(function(u){aux.getRange(u.row,2).setValue(u.value);});
  }

  // V4.7: la configuracion comercial SIEMPRE se guarda en CONFIGURACION y
  // se refleja en el panel AF:AG. Al guardar, se recalculan todos los pedidos,
  // montos, fletes, saldos, costo de produccion y resumen/proyeccion.
  const comercial={};
  ['precio1','precio2','precio3','precioExtra','costoUnitario','fleteLima','fleteProvincia','tipoCambioPublicidad','producto','um','colorMorado','colorAmarillo','colorRojo','colorAzul','colorVerde'].forEach(function(k){
    if(p[k]!==undefined) comercial[k]=p[k];
  });
  if(Object.keys(comercial).length){
    guardarConfiguracionNegocioV47_(comercial);
    if (typeof recalcularGastosPublicidadV472_ === 'function') recalcularGastosPublicidadV472_(obtenerLibro_());
    try { actualizarResumenVentasCore_(obtenerLibro_()); } catch(e){}
  }

  apiRegistrarAuditoria_(user,'ACTUALIZAR_CONFIG','', 'CONFIGURACION','Precios/costos/fletes y reglas operativas actualizados');
  apiInvalidarCachesV46_();
  return apiLeerConfigCompletaV45_();
}
function apiForzarSincronizacion_(user){const lock=LockService.getDocumentLock();if(!lock.tryLock(30000))throw new Error('Otra sincronizacion esta activa.');try{const libro=obtenerLibro_(),gestion=libro.getSheetByName(SC_CONFIG.HOJA_GESTION);if(gestion&&typeof asegurarConfiguracionNegocioV47_==='function')asegurarConfiguracionNegocioV47_(libro,gestion);const sync=sincronizarPedidosCore_(libro);v45ProcesarVencimientos_();if(sync.insertados>0)actualizarResumenVentasCore_(libro);apiRegistrarAuditoria_(user,'SINCRONIZACION_MANUAL','', 'SISTEMA','Nuevos insertados: '+sync.insertados);apiInvalidarCachesV46_();return sync;}finally{lock.releaseLock();}}
...
/** Diagnostico V4.6: no modifica pedidos. */
function pruebaRendimientoV46(){console.log('API '+SC_API.VERSION+' OK');console.log('CACHE_EPOCH='+apiEpoch46_());console.log('SESSION_EPOCH='+apiSessionEpoch46_());return true;}
