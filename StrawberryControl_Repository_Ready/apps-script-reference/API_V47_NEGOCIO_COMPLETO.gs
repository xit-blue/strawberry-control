/**
 * STRAWBERRY CONTROL V4.7 - API NEGOCIO COMPLETO + RENDIMIENTO
 * -----------------------------------------------------------------------------
...
function instalarRendimientoV47(){
  ...
  apiInvalidarCachesV46_();
  console.log('V4.7 ACTIVADA: rendimiento + vencimientos + sincronizacion Hoja 1 -> GESTION PEDIDOS.');
  return true;
}
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'getOrder': return apiJson_({ ok: true, order: apiObtenerPedidoV45_(params.pedido), history: apiListarHistorialLlamadas_(params.pedido, 100), maxAttempts: leerConfigWebV45_().maxIntentos });
...
function apiListarPedidosV45_(params) {
  ...
  if(!apiFresh46_(params)){const cached=apiCacheGetJson46_(cacheKey);if(cached)return cached;}
...
function apiForzarSincronizacion_(user){const lock=LockService.getDocumentLock();if(!lock.tryLock(60000))throw new Error('Otra sincronizacion esta activa.');try{const libro=obtenerLibro_(),gestion=libro.getSheetByName(SC_CONFIG.HOJA_GESTION);if(gestion&&typeof asegurarConfiguracionNegocioV47_==='function')asegurarConfiguracionNegocioV47_(libro,gestion);sincronizarPedidosCore_(libro);v45ProcesarVencimientos_();actualizarResumenVentasCore_(libro);apiRegistrarAuditoria_(user,'SINCRONIZACION_MANUAL','', 'SISTEMA','Hoja 1 -> GESTION PEDIDOS sincronizada manualmente');apiInvalidarCachesV46_();}finally{lock.releaseLock();}}
