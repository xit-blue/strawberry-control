/**
 * STRAWBERRY CONTROL V4.6 - API OPTIMIZADA PARA ASP.NET CORE + GOOGLE SHEETS
 * -----------------------------------------------------------------------------
...
function apiForzarSincronizacion_(user){const lock=LockService.getDocumentLock();if(!lock.tryLock(60000))throw new Error('Otra sincronizacion esta activa.');try{sincronizarPedidosCore_(obtenerLibro_());v45ProcesarVencimientos_();actualizarResumenVentasCore_(obtenerLibro_());apiRegistrarAuditoria_(user,'SINCRONIZACION_MANUAL','', 'SISTEMA','Sincronizacion manual');apiInvalidarCachesV46_();}finally{lock.releaseLock();}}
