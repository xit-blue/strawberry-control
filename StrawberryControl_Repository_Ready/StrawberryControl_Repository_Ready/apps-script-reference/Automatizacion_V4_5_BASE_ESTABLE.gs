/**
 * STRAWBERRY CONTROL V4.5 - BASE ESTABLE + API WEB SEPARADA
 * -----------------------------------------------------------------------------
...
/**
 * Sincroniza todo en memoria y escribe una sola matriz.
 * Conserva los campos manuales y mantiene pedidos que ya no aparezcan en Hoja 1.
 */
function sincronizarPedidosCore_(libro) {
  ...
  // No se borran pedidos que desaparezcan del origen; se conservan al final.
  existentes.filas.forEach(function(registro) {
    const pedido = normalizarPedido_(registro.datos[SC_COL.PEDIDO]);
    if (!pedido || pedidosOrigen.has(pedido)) return;
    salida.push(recalcularFilaGestion_(registro.datos, configuracion));
  });

  const filasNecesarias = Math.max(
    SC_CONFIG.FILAS_PREPARADAS,
    salida.length + SC_CONFIG.FILAS_BUFFER
  );
  asegurarTamanoHoja_(gestion, filasNecesarias + 1, SC_CONFIG.COLUMNAS_TOTALES);
  asegurarFormatoGestionHasta_(gestion, filasNecesarias);

  const ultimaFilaAnterior = existentes.ultimaFila;
  if (salida.length > 0) {
    const rangoSalida = gestion.getRange(2, 1, salida.length, SC_CONFIG.COLUMNAS_GESTION);
    rangoSalida.clearDataValidations();
    rangoSalida.setValues(salida);
    aplicarValidacionesBaseGestion_(gestion, 2, salida.length);
    aplicarValidacionesEstadoDatos_(gestion, 2, salida);
  }

  const primeraFilaSobrante = salida.length + 2;
  if (ultimaFilaAnterior >= primeraFilaSobrante) {
    gestion
      .getRange(
        primeraFilaSobrante,
        1,
        ultimaFilaAnterior - primeraFilaSobrante + 1,
...
function manejarEdicionGestion_(evento) {
  ...
  const columnasRelevantes = [
    5, 6, 7,      // ubicacion, por si se corrige manualmente
    10,           // cantidad real
...
function manejarEdicionGastos_(evento) {
  ...
  actualizarTotalesGastosFilas_(hoja, filaInicial, cantidad);
...
function prepararGestion_(hoja) {
  ...
  try {
    hoja.showColumns(23, 4);
    hoja.hideColumns(27, 1);
  } catch (error) {
    console.log(error);
  }
...
function leerConfiguracion_(hoja) {
  ...
  // Compatibilidad V4.x: si existe CONFIGURACION, se prioriza.
  // Esto permite conservar los valores que ya pudieron migrarse desde AF:AG
  // sin reconstruir GESTION PEDIDOS.
  try {
    const config = libro.getSheetByName('CONFIGURACION');
    if (config && config.getLastRow() >= 2) {
      const datos = config.getRange(2, 1, config.getLastRow() - 1, 2).getValues();
      const mapa = {};
      datos.forEach(function(f) {
        if (f[0]) mapa[String(f[0]).trim()] = f[1];
      });
    ...
    console.log('CONFIGURACION no disponible, se usara el panel V3.6 o valores por defecto: ' + e);
  }
...
function actualizarTotalesGastosFilas_(hoja, filaInicial, cantidadFilas) {
  ...
  const cantidad = Math.min(
    maxDisponibles,
    Math.max(0, Math.floor(numeroSeguro_(cantidadFilas)))
  );
  if (cantidad <= 0) return;

  const importes = hoja.getRange(inicio, 3, cantidad, 2).getValues();
  const totales = importes.map(function(fila) {
    const fbVacio = estaVacio_(fila[0]);
    const tiktokVacio = estaVacio_(fila[1]);

    if (fbVacio && tiktokVacio) return [''];

    const fbSoles = Math.max(0, numeroSeguro_(fila[0]));
    const tiktokSoles = Math.max(0, numeroSeguro_(fila[1]));
    return [Math.round((fbSoles + tiktokSoles) * 100) / 100];
  });
...
// Alias interno para compatibilidad con cualquier llamada de versiones previas.
function instalarFormulasTotalesGastos_(hoja, filaInicial, cantidadFilas) {
  actualizarTotalesGastosFilas_(hoja, filaInicial, cantidadFilas);
}

function recalcularTotalesGastosCore_(hoja) {
  const cantidad = Math.min(
    SC_CONFIG.FILAS_GASTOS_PREPARADAS,
    Math.max(0, hoja.getMaxRows() - 1)
  );
  actualizarTotalesGastosFilas_(hoja, 2, cantidad);
}
